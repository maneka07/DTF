...
!  
!  Modified by tatiana 2018-08-14  (RIKEN-CCS)
      ...
      subroutine write_savefile
          ...
          use runtime_m,      only: method, tstep, time_save, io_one_species_at_a_time
          use topology_m,  	  only: gcomm, mypx, mypy, mypz
		  ...
		  call dtf_transfer(trim(filename)//CHAR(0), ncid, err)
      ...
      subroutine read_savefile
		  ...
		  use io_profiling_m
          use runtime_m,      only: time, time_ref, run_title
          use topology_m,     only: gcomm, mypx, mypy, mypz, myid
		  use param_m,        only: nx, ny, nz, nx_g, ny_g, nz_g, nsc
          use variables_m,    only: temp, pressure, yspecies, u
