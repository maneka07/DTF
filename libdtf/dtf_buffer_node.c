...
void print_nodes(buffer_node_t* nodes){
    ...
    while(tmp != NULL){
        DTF_DBG(VERBOSE_ALL_LEVEL, "node (%lld, %lld)", tmp->offset, tmp->data_sz);
        tmp = tmp->next;
    }
}

buffer_node_t* new_buffer_node(MPI_Offset offset, MPI_Offset data_sz, int dtf_malloc_flag)
{
    buffer_node_t *node = dtf_malloc(sizeof(buffer_node_t));
    assert(node != NULL);
    if(dtf_malloc_flag){
        node->data = dtf_malloc((size_t)data_sz);
...
{
    DTF_DBG(VERBOSE_ALL_LEVEL, "Insert node offt %d, datasz %d", (int)node->offset, (int)node->data_sz);
    if(*list == NULL){
        *list = node;
        return;
    }else{
