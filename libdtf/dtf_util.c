        ...
        switch(gl_conf.distr_mode){
            case DISTR_MODE_STATIC:
                for(i = 0; i < fbuf->distr_nranks; i++){
                    ...
                    DTF_DBG(VERBOSE_DBG_LEVEL,   "Notify reader rank %d that file %s is ready", dest, fbuf->file_path);
                ...
                break;
            default:
                DTF_DBG(VERBOSE_ERROR_LEVEL, "DTF Error: 4 unknown data distribution mode");
...
void close_file(file_buffer_t *fbuf)
{
    ...
    if(fbuf->writer_id == gl_my_comp_id){
        ...
        if((fbuf->iomode == DTF_IO_MODE_FILE) && (gl_conf.distr_mode == DISTR_MODE_STATIC)){
