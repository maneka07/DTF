                               ...
                               int rw_flag)
{
    ...
    assert(el_sz > 0);
    DTF_DBG(VERBOSE_ERROR_LEVEL, "Mismatch of var's datatype");
    ...
    if(var->ndims <= 1){ /*scalar or 1d array*/
        ...
        if(ret != data_sz)
            DTF_DBG(VERBOSE_WARNING_LEVEL, "DTF Warning: Meant to read/write %llu bytes but actual val is %llu", data_sz, ret);
...
static void unpack_vars(file_buffer_t *fbuf, int buf_sz, void *buf, MPI_Offset *node_cnt, MPI_Offset **first_el_coord)
{
    ...
    *first_el_coord = NULL;
    ...
    DTF_DBG(VERBOSE_DBG_LEVEL, "Unpacking vars sz %d", buf_sz);
        ...
        if(fbuf->distr_pattern == DISTR_PATTERN_ALL) {
            ...
            *node_cnt += ncnt;
            ...
            if(ncnt != 0) {
                ...
                var->distr_count = (MPI_Offset*)dtf_malloc(var->ndims*sizeof(MPI_Offset));
                assert(var->distr_count != NULL);
    ...
    DTF_DBG(VERBOSE_DBG_LEVEL, "Unpacking vars: offt is %lu, bufsz is %lu", offt, (size_t)buf_sz);
    assert(offt == (size_t)buf_sz);
    DTF_DBG(VERBOSE_DBG_LEVEL, "Finished unpacking vars");
}
...
int receive_data(file_buffer_t *fbuf, int rank, MPI_Comm intercomm)
{
    ...
    if(fbuf->distr_pattern == DISTR_PATTERN_ALL) {
        DTF_DBG(VERBOSE_DBG_LEVEL, "Will recv %d nodes", (int)node_cnt);
        MPI_Request *reqs = NULL;
        reqs = (MPI_Request*)dtf_malloc(sizeof(MPI_Request)*node_cnt);
        assert(reqs != NULL);
        ncnt = 0;
        ...
        while(var != NULL){
            ...
            while(node != NULL){
                ...
                DTF_DBG(VERBOSE_ALL_LEVEL, "Receive node sz %d offt %d", (int)node->data_sz, (int)node->offset);
                node->data = dtf_malloc((size_t)node->data_sz);
        ...
        CHECK_MPI(errno);
        dtf_free(reqs, sizeof(MPI_Request)*node_cnt);
        ...
        while(var != NULL){
            ...
            gl_conf.malloc_size += bufsz;
            rbuf = tmp;
            DTF_DBG(VERBOSE_ALL_LEVEL, "Try to recv node of sz %d", (int)bufsz);
...
static void pack_vars(file_buffer_t *fbuf, int dst_rank, int *buf_sz, void **buf, MPI_Offset *node_cnt, MPI_Offset **first_el_coord)
{
    ...
    *first_el_coord = NULL;
    ...
    DTF_DBG(VERBOSE_DBG_LEVEL, "Packing vars: sz %lu", sz);
            ...
            */
            ...
            if(var->distr_count == NULL){
                /*If distr_count wasn't set we won't be distributed this variable to
                  ...
                  DTF_DBG(VERBOSE_WARNING_LEVEL, "DTF Warning: dtf_set_distr_count was not called for variable with id %d. \
                ...
                *first_el_coord = coord;
                ...
                for(j = 0; j < var->ndims; j++){
                    DTF_DBG(VERBOSE_ALL_LEVEL, "start with coord %d", (int)coord[j]);
                }
                offset = var->nodes->offset;
                for(rank_idx = 0; rank_idx < fbuf->distr_nranks; rank_idx++){
                    while(1){
                        index_1d = to_1d_index(var->ndims, var->shape, coord);
                        DTF_DBG(VERBOSE_ALL_LEVEL, "1d idx %d", (int)index_1d);
                        /*Check that we haven' t gone out of bounds*/
                        if(index_1d > last_el_idx){
                            DTF_DBG(VERBOSE_ERROR_LEVEL, "DTF Error: There is no data to send to rank %d. Incorrect config file?", dst_rank);
                            MPI_Abort(MPI_COMM_WORLD, MPI_ERR_OTHER);
                        }
                        offset = (MPI_Offset)el_sz * index_1d;
                        DTF_DBG(VERBOSE_ALL_LEVEL, "offset %d", (int)offset);
                ...
                for(j = 0; j < var->ndims; j++){
                    DTF_DBG(VERBOSE_ALL_LEVEL, "data starts at %d, count %d, shape %d", (int)coord[j], (int)var->distr_count[j], (int)var->shape[j]);
                }
                DTF_DBG(VERBOSE_DBG_LEVEL, "Will send to rank %d data starting at idx %d", dst_rank, (int)index_1d);
    ...
    *buf_sz = (int)offt;
    DTF_DBG(VERBOSE_DBG_LEVEL, "Actually packed %d", *buf_sz);
    DTF_DBG(VERBOSE_DBG_LEVEL, "Finish packing vars");
}
...
int set_distr_count(file_buffer_t *fbuf, int varid, int count[])
{
    ...
    if(fbuf->distr_pattern != DISTR_PATTERN_SCATTER){
        DTF_DBG(VERBOSE_WARNING_LEVEL, "DTF Warning: cannot set distribute count. File's distribute pattern must be <scatter>.");
        return 0;
    }

    dtf_var_t *var = find_var(fbuf->vars, varid);
    if(var == NULL){
        DTF_DBG(VERBOSE_ERROR_LEVEL, "DTF Error: set_distr_count(): No variable with such id (%d). Ignored.", varid);
        return 1;
    }
    assert(var->distr_count == NULL);
    var->distr_count = (MPI_Offset*)dtf_malloc(var->ndims*sizeof(MPI_Offset));
...
int send_data(file_buffer_t *fbuf, int rank, MPI_Comm intercomm)
{
    ...
    if(fbuf->distr_pattern == DISTR_PATTERN_ALL) {
        ...
        while(var != NULL){
            ...
            node = var->nodes;
            DTF_DBG(VERBOSE_DBG_LEVEL, "Will send %d nodes", (int)var->node_cnt);
            for(i = 0; i < var->node_cnt; i++){
                DTF_DBG(VERBOSE_ALL_LEVEL, "node sz %d offt %d", (int)node->data_sz, (int)node->offset);
        ...
        while(var != NULL){
            ...
            gl_conf.malloc_size += bufsz;
            for(i = 0; i < var->ndims; i++){
                DTF_DBG(VERBOSE_ALL_LEVEL, "send data starts at %d", (int)first_el_coord[first_el_coord_sz+i]);
            }
            readsz = mem_noncontig_read(var, &first_el_coord[first_el_coord_sz], var->distr_count, sbuf);
            if( (int)readsz != bufsz ){
                DTF_DBG(VERBOSE_ERROR_LEVEL, "DTF Warning: not enough data to send to rank %d", rank);
                DTF_DBG(VERBOSE_DBG_LEVEL, "Should read %d but read %d", (int)bufsz, (int)readsz);
            }
            //if(readsz != 0){
            DTF_DBG(VERBOSE_ALL_LEVEL, "Sending node of sz %d", (int)readsz);
    ...
    DTF_DBG(VERBOSE_DBG_LEVEL, "Total sent mem nodes %d, should have sent %d", ncnt, (int)node_count);
