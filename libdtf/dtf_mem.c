...
//static MPI_Offset mem_recursive_write(dtf_var_t *var, const MPI_Offset start[], const MPI_Offset count[], void *data, int dim, MPI_Offset coord[])
//{
//    int i;
//    MPI_Offset written = 0;
//    if(dim == var->ndims - 1){
//        MPI_Offset *dst_coord = (MPI_Offset*)dtf_malloc(var->ndims*sizeof(MPI_Offset));
...
//        DTF_DBG(VERBOSE_ALL_LEVEL, "will write %llu bytes from el %llu (%llu)", data_sz, src_start_idx, src_offt);
...
MPI_Offset mem_noncontig_write(dtf_var_t *var, const MPI_Offset start[], const MPI_Offset count[], void *data)
{
    MPI_Offset written = 0;
    ...
    while(chunks != NULL){
        ...
        tmp = chunks;
        chunks = chunks->next;
        dtf_free(tmp, sizeof(contig_mem_chunk_t));
    }
    if((var->ndims > 0) && (nelems > 0)){
        if(var->first_coord == NULL){
            var->first_coord = (MPI_Offset*)dtf_malloc(var->ndims*sizeof(MPI_Offset));
...
MPI_Offset mem_contiguous_write(dtf_var_t *var, MPI_Offset offset, MPI_Offset data_sz, void *data)
{
    ...
    buffer_node_t *node;
    DTF_DBG(VERBOSE_ALL_LEVEL,   "Will write %llu bytes to offt %llu", data_sz, offset);
    ...
    if(var->nodes == NULL){
        ...
        DTF_DBG(VERBOSE_DBG_LEVEL, "Leave mem_write");
        return data_sz;
    }
    ...
    DTF_DBG(VERBOSE_ALL_LEVEL, "found node with offt %d", (int)node->offset);
    ...
    while(copied != data_sz){
        ...
        if(offset < node->offset){
            ...
            var->node_cnt++;
            DTF_DBG(VERBOSE_ALL_LEVEL, "Copy %d to node with offt %d (sz %d) at node offt 0",
...
static void traverse_dims(dtf_var_t *var,
                          MPI_Datatype dtype,
                          ...
                          contig_mem_chunk_t **list)
{
    ...
    if(dim == var->ndims - 1){
        ...
        contig_mem_chunk_t *new_chunk = (contig_mem_chunk_t*)dtf_malloc(sizeof(contig_mem_chunk_t));
        ...
        DTF_DBG(VERBOSE_ALL_LEVEL,"Added a tuple (off %d, uoff %d, sz %d)", (int)new_chunk->offset, (int)new_chunk->usrbuf_offset, (int)new_chunk->data_sz);
        return;
    }
...
MPI_Offset mem_contiguous_read(dtf_var_t *var, MPI_Offset offset,  MPI_Offset data_sz, void *data)
{
   ...
   // MPI_Offset start_idx = to_1d_index(var->ndims, var->shape, first_el_coord); //el offset within var->nodes
    ...
    DTF_DBG(VERBOSE_ALL_LEVEL, "will read %llu bytes from offt %llu", data_sz, offset);
    ...
    if(tmp == NULL){
        DTF_DBG(VERBOSE_ERROR_LEVEL, "DTF Error: there is no data at offset %llu", offset);
        return 0;
    }
    copied = 0;
    ...
    while(copied != data_sz){
        ...
        DTF_DBG(VERBOSE_ALL_LEVEL, "From node with offt %d Will copy %d bytes from node_offt %d", (int)tmp->offset, (int)to_cpy, (int)node_offt);
