...
static read_dblock_t* new_rblock(int var_id, MPI_Offset* start, MPI_Offset* count, int ndims)
{
	...
	if(ndims > 0){
		...
		dblock->start = dblock->count = NULL;
	DTF_DBG(VERBOSE_DBG_LEVEL, "create new rblock");
	return dblock;
}
...
static void do_matching(file_buffer_t *fbuf)
{
    ...
    while(ritem != NULL){
        ...
        while(rblock != NULL){
			...
			//~ else {
				...
				//~ while(wblock != NULL){
					//~ int match = 0;
					//~ for(i = 0; i < ndims; i++)
						//~ if((rblock->start[i] >= wblock->start[i]) && (rblock->start[i] < wblock->start[i] + wblock->count[i]))
							//~ match++;
						//~ else 
							//~ break;
					//~ if(match == ndims)
			...
			/*In case the found block only partially overlaps
			 * with the requested block, create separate request
			 * for all sub-blocks that are left, queue them right 
			 * after current read block*/
			for(i = 0; i < ndims; i++){
				if(rblock->start[i] < wblock->start[i]){
					read_dblock_t* new_block = new_rblock(var_id, rblock->start, rblock->count, ndims);
					ritem->nblocks++;
					new_block->count[i] = wblock->start[i] - rblock->start[i];
					new_block->prev = rblock;
					new_block->next = rblock->next;
					if(rblock->next != NULL)
						rblock->next->prev = new_block;
					rblock->next = new_block;					
				}
				if(rblock->start[i] + rblock->count[i] > wblock->start[i] + wblock->count[i]){
					read_dblock_t* new_block = new_rblock(var_id, rblock->start, rblock->count, ndims);
					ritem->nblocks++;
					new_block->start[i] = wblock->start[i] + wblock->count[i];
					new_block->count[i] = rblock->start[i] + rblock->count[i] - new_block->start[i];
					new_block->prev = rblock;
					new_block->next = rblock->next;
					if(rblock->next != NULL)
						rblock->next->prev = new_block;
				...
				/*adjust coordinate and count for the overlapping part in this dimension*/
				if(rblock->start[i] < wblock->start[i])
					rblock->start[i] = wblock->start[i];
				if(rblock->start[i] + rblock->count[i] > wblock->start[i] + wblock->count[i])
					rblock->count[i] = wblock->start[i] + wblock->count[i] - rblock->start[i];
			}
			...
			if(i == nwriters){ 
				...
				/*add new send buffer*/
				void *tmp;
				tmp = realloc((void*)writers, (nwriters+1)*sizeof(int));
				assert(tmp != NULL);
				writers = (int*)tmp;
				
				tmp = realloc((void*)offt, (nwriters+1)*sizeof(size_t));
				assert(tmp != NULL);
				offt = (size_t*)tmp;				

				tmp = realloc((void*)bufsz, (nwriters+1)*sizeof(size_t));
				assert(tmp != NULL);
				bufsz = (size_t*)tmp;
				
				unsigned char** tmp1;
				tmp1 = realloc(sbuf, (nwriters+1)*sizeof(unsigned char*));
				assert(tmp1 != NULL);
				sbuf = tmp1;
				
				gl_proc.stats_info.malloc_size += sizeof(int) + sizeof(unsigned char*) + sizeof(size_t)*2;
				
				offt[nwriters] = 0;
				sbuf[nwriters] = NULL;
				bufsz[nwriters] = 0;
				writers[nwriters] = wblock->rank;
				rank_idx = nwriters;
				nwriters++;
				DTF_DBG(VERBOSE_DBG_LEVEL, "Created sbuf for %d", rank_idx); 
			} else
				rank_idx = i;
			...
			if(offt[rank_idx] == 0) {
				if(bufsz[rank_idx] < MAX_FILE_NAME + sizeof(MPI_Offset)){
					//extend
					unsigned char *tmp;
					tmp = realloc(sbuf[rank_idx], bufsz[rank_idx] + MAX_FILE_NAME + 1024);
					assert(tmp != NULL);
					sbuf[rank_idx] = tmp;
					bufsz[rank_idx] += MAX_FILE_NAME + 1024;
				}
			 ...
			 if(offt[rank_idx] + sizeof(MPI_Offset)*ndims*2 +sizeof(MPI_Offset)> bufsz[rank_idx]){
				unsigned char *tmp;
				tmp = realloc(sbuf[rank_idx], bufsz[rank_idx] + 1024);
				assert(tmp != NULL);
				sbuf[rank_idx] = tmp;
				bufsz[rank_idx] += 1024;
			}
			
			DTF_DBG(VERBOSE_ALL_LEVEL, "Saving matched block:");
			for(i = 0; i < var->ndims; i++)
				DTF_DBG(VERBOSE_ALL_LEVEL, "%lld -> %lld",
						rblock->start[i], rblock->count[i]);
			...
			*(MPI_Offset*)(sbuf[rank_idx] + offt[rank_idx]) = (MPI_Offset)rblock->var_id;
			...
			if(ndims > 0){
				...
				memcpy(sbuf[rank_idx]+offt[rank_idx], rblock->count, ndims*sizeof(MPI_Offset));
				offt[rank_idx] += ndims*sizeof(MPI_Offset);
			}
			
			assert(offt[rank_idx] <= bufsz[rank_idx]);
			
			read_dblock_t *tmp = rblock;
			rblock = rblock->next;
			
			DEQUEUE_ITEM(tmp,  ritem->dblocks);
			if(ndims > 0){
				dtf_free(tmp->start, ndims*sizeof(MPI_Offset));
				dtf_free(tmp->count, ndims*sizeof(MPI_Offset));
			}
			dtf_free(tmp, sizeof(read_dblock_t));
			ritem->nblocks--;
			DTF_DBG(VERBOSE_ALL_LEVEL, "left %lu blocks", ritem->nblocks);
			if(ritem->nblocks == 0) 
				assert(ritem->dblocks == NULL);
        }
        ...
        if(nwriters > 0){
            ...
            for(i = 0; i < nwriters; i++){
                ...
                if(writers[i] == gl_proc.myrank){
                    ...
                    err = MPI_Isend((void*)sbuf[i], (int)offt[i], MPI_BYTE, writers[i], IO_DATA_REQ_TAG, gl_proc.comps[gl_proc.my_comp].comm, msg->reqs);
                    CHECK_MPI(err);
            ...
            if(my_idx != -1){
                ...
                send_data_nonblocking(fbuf, sbuf[my_idx] + MAX_FILE_NAME, offt[my_idx] - MAX_FILE_NAME);
                dtf_free(sbuf[my_idx], bufsz[my_idx]);
            }
            DTF_DBG(VERBOSE_DBG_LEVEL, "Matched rreq for rank %d", ritem->rank);
            /*dealloc stuff*/
			dtf_free(writers, nwriters*sizeof(int));
			dtf_free(offt, nwriters*sizeof(size_t));
			dtf_free(bufsz, nwriters*sizeof(size_t));
			dtf_free(sbuf, nwriters*sizeof(unsigned char*));
			sbuf = NULL;
			bufsz = offt = NULL;
			writers = NULL;
			nwriters = 0;
        ...
        if(ritem->nblocks == 0){
            ...
            DEQUEUE_ITEM(tmp, fbuf->my_mst_info->iodb->ritems);  
...
static void parse_ioreqs(file_buffer_t *fbuf, void *buf, int bufsz, int src_rank, MPI_Comm comm)
{
    int var_id, rw_flag;
    dtf_var_t *var = NULL;
    size_t offt = 0;
    ...
    while(offt != (size_t)bufsz){
		...
		DTF_DBG(VERBOSE_DBG_LEVEL, "var %d, %d dims", var_id, var->ndims);

        if(rw_flag == DTF_READ){
			
			if(ritem == NULL || ritem->rank != src_rank){		
				ritem = fbuf->my_mst_info->iodb->ritems;
				while(ritem != NULL){
					if(ritem->rank == src_rank)
						break;
					ritem = ritem->next;
				}
			}
			...
			if(ritem == NULL){
				ritem = dtf_malloc(sizeof(read_db_item_t));
				ritem->next = NULL;
				ritem->prev = NULL;
				ritem->rank = src_rank;
				ritem->dblocks = NULL;
				...
				ENQUEUE_ITEM(ritem, fbuf->my_mst_info->iodb->ritems);
				fbuf->my_mst_info->iodb->nritems++;
			}
            ...
            read_dblock_t* dblock = new_rblock(var_id, start, count, var->ndims);
            
            //~ DTF_DBG(VERBOSE_ALL_LEVEL, "Added dblock for var %d:", var_id);
			//~ for(i = 0; i < var->ndims; i++)
				//~ DTF_DBG(VERBOSE_ALL_LEVEL, "%lld  ->  %lld", dblock->start[i], dblock->count[i]);
			
			ENQUEUE_ITEM(dblock, ritem->dblocks);
            ritem->nblocks++;
            ...
            if(fbuf->my_mst_info->iodb->witems[var_id] == NULL){
                ...
                witem = fbuf->my_mst_info->iodb->witems[var_id];
                witem->bl_tree = IntervalTreeCreate();
                witem->nblocks = 0;
			...
			block_t* bl =  dtf_malloc(sizeof(block_t));
			...
			if(var->ndims == 0){
				...
				bl->start = dtf_malloc(var->ndims*sizeof(MPI_Offset));
				bl->count = dtf_malloc(var->ndims*sizeof(MPI_Offset));
				memcpy(bl->start, chbuf+offt, var->ndims*sizeof(MPI_Offset));
				offt += var->ndims*sizeof(MPI_Offset);
				memcpy(bl->count, chbuf+offt, var->ndims*sizeof(MPI_Offset));
				offt += var->ndims*sizeof(MPI_Offset);
			}
			bl->next = witem->blocks;
			witem->blocks = bl;
			
			info.ndims = var->ndims;
			info.bl = bl;
						
			IntervalTreeInsert(witem->bl_tree, &info);

			DTF_DBG(VERBOSE_ALL_LEVEL, "Added dblock for var %d:", var_id);
			for(i = 0; i < var->ndims; i++)
				DTF_DBG(VERBOSE_ALL_LEVEL, "%lld  ->  %lld",bl->start[i], bl->count[i]);    
        }
    }
...
//    double t_part3=MPI_Wtime();
	...
	if(replay){
		...
		if(gl_proc.my_comp == fbuf->writer_id){ 
			...
			while(!fbuf->done_matching_flag) progress_comm(0);
...
void send_data_nonblocking(file_buffer_t *fbuf, void* buf, int bufsz)
{
    ...
    MPI_Offset fit_nelems;
	int type_mismatch;
	int cnt_mismatch;
	
    /*First compute how big a buffer we need to send data. 
	 * If the buffer is bigger than data_msg_size_limit then switch to 
	 * blocking sending of data.*/
	 
	sbufsz = MAX_FILE_NAME;
	rofft = sizeof(MPI_Offset);
	while(rofft != (size_t)bufsz){
		var_id = (int)(*(MPI_Offset*)(rbuf+rofft));
        rofft += sizeof(MPI_Offset);
        ...
        var = fbuf->vars[var_id];
		MPI_Type_size(var->dtype, &def_el_sz);
		rofft += var->ndims*sizeof(MPI_Offset); //skip start
		count = (MPI_Offset*)(rbuf+rofft);
        rofft += var->ndims*sizeof(MPI_Offset);
		int tmp = 1;
		for (i = 0; i < var->ndims; i++)
			tmp *= count[i]; 
		sbufsz += sizeof(MPI_Offset)*(var->ndims*2 + 1) + tmp*def_el_sz + (tmp*def_el_sz) % sizeof(MPI_Offset);
	}
    ...
    while(rofft != (size_t)bufsz){
		...
		*(MPI_Offset*)(sbuf+sofft) = (MPI_Offset)var_id;
		...
		/*Copy data*/
		fit_nelems = 1;
		for(i = 0; i < var->ndims; i++) fit_nelems *= count[i];
		...
		if(cnt_mismatch == 0){//it's a continious block of memory
			...
			if(gl_proc.conf.do_checksum){
				double chsum = compute_checksum(sbuf+sofft, var->ndims, count, var->dtype);
				DTF_DBG(VERBOSE_DBG_LEVEL, "chsum contin for req %lu is %.4f", ioreq->id, chsum);
			}
		} else {
			//adjust start coordinate with respect to the user buffer
			...
			if(ioreq->derived_params == NULL)
				get_put_data(var->ndims, ioreq->dtype, var->dtype, ioreq->count, ioreq->user_buf, 
							 rel_start, count, sbuf+sofft, DTF_READ, type_mismatch);
			else{
				DTF_DBG(VERBOSE_DBG_LEVEL, "Will extract data from derived data type");
				//adjust with respect to shift of subarray inside to original array
				for(i = 0; i < var->ndims; i++)
				...
				get_put_data(var->ndims, ioreq->dtype, var->dtype, 
							ioreq->derived_params->orig_array_size, ioreq->user_buf, rel_start, count, sbuf+sofft, DTF_READ, type_mismatch);
			}
			dtf_free(rel_start, var->ndims*sizeof(MPI_Offset));
    ...
    if(sofft != sbufsz){
		DTF_DBG(VERBOSE_DBG_LEVEL, "sofft %lu, sbufsz %lu", sofft, sbufsz);
		assert(sofft == sbufsz);
	}
	
	int flag;
	DTF_DBG(VERBOSE_DBG_LEVEL, "Send msg to %d size %d", rdr_rank,(int)sofft);
	dtf_msg_t *msg = new_dtf_msg(sbuf, sbufsz, DTF_UNDEFINED, IO_DATA_TAG, 1);
	
	double t_start_comm = MPI_Wtime();
	err = MPI_Isend((void*)sbuf, (int)sofft, MPI_BYTE, rdr_rank, IO_DATA_TAG, gl_proc.comps[fbuf->reader_id].comm, msg->reqs);
	CHECK_MPI(err); 
...
{
	int dbg = VERBOSE_DBG_LEVEL;
    switch(tag){
        case FILE_READY_TAG:
            DTF_DBG(dbg, "Received tag FILE_READY_TAG from %d", src);
            break;
        case IO_DATA_REQ_TAG:
            DTF_DBG(dbg, "Received tag IO_DATA_REQ_TAG from %d", src);
            break;
        case READ_DONE_TAG:
            DTF_DBG(dbg, "Received tag READ_DONE_TAG from %d", src);
            break;
        case FILE_INFO_TAG:
            DTF_DBG(dbg, "Received tag FILE_INFO_TAG from %d", src);
            break;
        case FILE_INFO_REQ_TAG:
            DTF_DBG(dbg, "Received tag FILE_INFO_REQ_TAG from %d", src);
            break;
        case MATCH_DONE_TAG:
            DTF_DBG(dbg, "Received tag MATCH_DONE_TAG from %d", src);
            break;
        case DONE_MULTIPLE_TAG:
            DTF_DBG(dbg, "Received tag DONE_MULTIPLE_TAG from %d", src);
            break;
        case IO_REQS_TAG:
            DTF_DBG(dbg, "Received tag IO_REQS_TAG from %d", src);
            break;
        case IO_DATA_TAG:
            DTF_DBG(dbg, "Received tag IO_DATA_TAG from %d", src);
            break;
		case COMP_FINALIZED_TAG:
			DTF_DBG(dbg, "Received tag COMP_FINALIZED_TAG from %d", src);
			break;	
        default:
            DTF_DBG(dbg, "Received tag unknown %d from %d", tag, src);
            assert(0);
    }
}
	...
	switch(tag){
		...
		case IO_DATA_REQ_TAG:
			...
			if(gl_proc.conf.use_msg_buffer)
				send_data_blocking(fbuf, (unsigned char*)rbuf+offt, bufsz - offt);
			else
				send_data_nonblocking(fbuf, (unsigned char*)rbuf+offt, bufsz - offt);
			break;
		case IO_DATA_TAG:
			if(!fbuf->is_transferring) goto fn_exit;  //Not ready to process this message yet
...
void progress_comm(int ignore_idle)
{
    ...
    for(comp = 0; comp < gl_proc.ncomps; comp++){
        ...
        if(gl_proc.comps[comp].comm == MPI_COMM_NULL)
            continue;
