...
int main()
{
  ...
  int *data_out;
   ...
   data_out = malloc(nranks*NY*sizeof(int));
   ...
   for (x = 0; x < nranks; x++)
	...
	data_out[x*NY+y] = bb++; //x * NY + y + myrank;
   //   	 data_out[x][y] = x * NY + y + myrank;
     }


   for (x = 0; x < nranks; x++)
     for (y = 0; y < NY; y++)
       printf("w%d:%d ",myrank, data_out[x*NY+y]);
   printf("\n");
   ...
   if ((retval = nc_def_dim(ncid, "x", nranks, &x_dimid)))
      ERR(retval);
   ...
   //   if ((retval = nc_put_var_int(ncid, varid, &data_out[0][0])))
     if ((retval = nc_put_var_int(ncid, varid, data_out)))
      ERR(retval);
