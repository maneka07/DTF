...
int main(int argc, char** argv) {
  ...
  /* open an existing file created earlier for read -----------------------*/
    ...
    buf[0] = (double*)  malloc(global_ny * myNX * sizeof(double));
    for (i=0; i<myNX; i++) {
        if (i > 0) buf[i] = buf[i-1] + global_ny;
        for (j=0; j<global_ny; j++) buf[i][j] = -1.0;
    }
    ...
    for (i=0; i<myNX; i++) {
        ...
        for (j=0; j<global_ny; j++){
            if (buf[i][j] != (double)rank+10) {
				printf("%d: Read contents mismatch at buf[%d][%d] = %.10f (should be %.10f)\n", rank, i,j,buf[i][j],rank+10.1);
            }
