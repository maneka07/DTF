...
void process_file_info_req_queue()
{
    ...
    while(req != NULL){
        ...
        if(gl_my_rank == fbuf->root_writer){
            FARB_DBG(VERBOSE_DBG_LEVEL, "I am root writer, process the file info req for file %s", req->filename);
            ...
            int errno;
            FARB_DBG(VERBOSE_DBG_LEVEL, "Forward the request to root writer %d", fbuf->root_writer);
        ...
        farb_free(req->buf, MAX_FILE_NAME+sizeof(int)*2);
        farb_free(req, sizeof(file_info_req_q_t));
        req = tmp;
        FARB_DBG(VERBOSE_ALL_LEVEL, "Freed request");
    }
}
    ...
    for(i = 0; i < gl_ncomp; i++){
        ...
        switch(status.MPI_TAG){
            case FILE_INFO_REQ_TAG:
                FARB_DBG(VERBOSE_DBG_LEVEL, "Recv root req from rank %d (comp %s)", src, gl_comps[i].name);
...
//            }
            if(fbuf->root_reader != -1){
                FARB_DBG(VERBOSE_DBG_LEVEL,   "Notify reader root rank %d that file %s is ready", fbuf->root_reader, fbuf->file_path);
...
void close_file(file_buffer_t *fbuf)
{
    ...
    if(fbuf->writer_id == gl_my_comp_id){
        ...
        if((fbuf->iomode == FARB_IO_MODE_FILE) && (gl_conf.distr_mode == DISTR_MODE_STATIC)){
            ...
            FARB_DBG(VERBOSE_DBG_LEVEL, "Reader hasn't closed the file yet. Waiting...");
            while( !fbuf->rdr_closed_flag)
                progress_io_matching();
            FARB_DBG(VERBOSE_DBG_LEVEL, "Cleaning up everything");
            delete_ioreqs(fbuf);
        ...
        if(fbuf->iomode == FARB_IO_MODE_FILE){
            ...
            if(fbuf->root_writer == -1){
                if(rank == 0){
                    ...
                    void *buf = farb_malloc(MAX_FILE_NAME+2*sizeof(int));
                    assert(buf != NULL);
                    memcpy(buf, fbuf->file_path, MAX_FILE_NAME);
                    memcpy((unsigned char*)buf+MAX_FILE_NAME, &gl_my_rank, sizeof(int));
                    FARB_DBG(VERBOSE_DBG_LEVEL, "Asking writer who is the root of file %s", fbuf->file_path);
                    errno = MPI_Send(buf, (int)(MAX_FILE_NAME+2*sizeof(int)), MPI_BYTE, 0, FILE_INFO_REQ_TAG, gl_comps[fbuf->writer_id].comm);
                    CHECK_MPI(errno);
                    farb_free(buf, MAX_FILE_NAME+2*sizeof(int));

                    FARB_DBG(VERBOSE_DBG_LEVEL, "Starting to wait for file info for %s", fbuf->file_path);
                    ...
                    FARB_DBG(VERBOSE_DBG_LEVEL, "Root for file %s is %d", fbuf->file_path, fbuf->root_writer);
                }
            ...
            if(gl_conf.distr_mode == DISTR_MODE_REQ_MATCH){
                ...
                if(fbuf->root_writer != -1){
                    FARB_DBG(VERBOSE_DBG_LEVEL, "FARB Warning: We already know the masters for this file from, e.g., a previous iteration");
                    goto fn_exit;
                }
                MPI_Comm_rank(comm, &rank);
                /*Zero rank will inquire the pnetcdf header/farb vars info/masters info
                from writer's global zero rank and then broadcast this info to other
                readers that opened the file*/
                if(rank == 0){
                    buf = farb_malloc(MAX_FILE_NAME+2*sizeof(int));
                    assert(buf != NULL);
                    memcpy(buf, fbuf->file_path, MAX_FILE_NAME);
                    memcpy((unsigned char*)buf+MAX_FILE_NAME, &gl_my_rank, sizeof(int));
               ...
               // MPI_Barrier(comm);
                ...
                farb_free(buf, bufsz);
                fbuf->is_ready = 1;
            } else if(gl_conf.distr_mode == DISTR_MODE_STATIC){
                while(!fbuf->is_ready)
                    progress_io();
            }
        }
